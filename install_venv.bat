...
@echo.
@echo.
rem Спрашиваем пользователя готов ли он запустить программу:
rem если да, то переходим на метку start, если нет завершаем скрипт
set answer=нет
set /P answer=Хотите запустить приложение прямо сейчас? (Да/[Нет])
    rem Возврат исходной кодировки
    chcp %ccp% > nul
if /I "%answer:~0,1%"=="д" (
    goto :start
) else (
    chcp 65001 > nul && echo Для запуска приложения используйте start.bat
    goto :EOF
)
...
chcp 65001 > nul
cls
echo Для запуска приложения в следующий раз используйте start.bat
echo Запуск...
rem Возврат исходной кодировки
chcp %ccp% > nul
call start.bat
